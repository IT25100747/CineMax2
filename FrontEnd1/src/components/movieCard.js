// // filepath: src/components/movieCard.js
...
//         ? '<div class="absolute inset-0 bg-black/50 flex items-center justify-center"><div class="bg-amber-500 text-black text-sm px-3 py-1 rounded-full font-bold -rotate-3">Coming Soon</div></div>' 
//         : '<div class="absolute bottom-0 left-0 right-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300"><button class="w-full bg-red-600 hover:bg-red-500 text-white py-2 rounded-lg text-sm font-semibold">Book Now</button></div>'}
//     </div>
//     <div class="mt-3 space-y-1">
//       <h3 class="text-white truncate font-semibold">${movie.title}</h3>
//       <div class="flex items-center gap-3 text-white/50 text-xs">
//         <span class="flex items-center gap-1">${icon('clock','w-3 h-3')} ${movie.duration}</span>
//         ${movie.status === 'coming-soon' ? `<span class="flex items-center gap-1">${icon('calendar','w-3 h-3')} ${movie.releaseDate}</span>` : ''}
//       </div>
...
// /**
//  * Generates genre tags HTML for a movie (imported from helpers)
